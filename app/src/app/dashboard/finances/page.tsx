...
export default async function FinancesPage() {
  ...
  const statusConfig: Record<string, { className: string; label: string }> = {
    ...
    void: { className: "bg-[#262118] text-[#8C7D6A] ring-1 ring-inset ring-[#312B20]", label: "Void" },
  };
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="hidden sm:flex items-center gap-2">
          <Link
            ...
            className="flex items-center gap-1.5 px-3 py-2 text-[12px] font-semibold text-[#8C7D6A] bg-[#1F1B14] border border-[#312B20] rounded-[6px] hover:bg-[#262118] transition-colors"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 bg-[#1F1B14] rounded-[6px] border border-[#312B20] shadow-[0_1px_3px_rgba(0,0,0,0.4)] overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#262118]">
            <h2 className="text-[14px] font-bold text-[#E8DFD0]">
              Recent Transactions
            </h2>
            <Link
              href="/dashboard/invoices"
              className="flex items-center gap-1 text-[12px] font-semibold text-[#5A5040] hover:text-[#E8DFD0] transition-colors"
            >
              View All
          ...
          {recentInvoices.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center px-6">
              <div className="h-12 w-12 rounded-[6px] bg-white/[0.04] flex items-center justify-center mb-3">
                <Receipt className="h-6 w-6 text-[#4E4235]" strokeWidth={1.5} />
              </div>
              <p className="text-[13px] font-semibold text-[#8C7D6A]">
                No transactions yet
              </p>
              <p className="text-[12px] text-[#5A5040] mt-1 max-w-xs">
                Create your first invoice to start tracking revenue
              </p>
              <Link
                href="/dashboard/invoices"
                className="mt-4 flex items-center gap-1.5 px-4 py-2 text-[12px] font-semibold bg-[#F59E0B] text-white rounded-[6px] hover:bg-[#F59E0B]/90 transition-colors"
              >
                Create Invoice
                ...
                return (
                  <div key={inv.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-white/[0.02] transition-colors">
                    <div className={`h-8 w-8 rounded-[6px] flex items-center justify-center shrink-0 ${
                      inv.status === "paid" ? "bg-[#34C759]/10" : inv.status === "unpaid" ? "bg-[#FF9F0A]/10" : "bg-[#262118]"
                    }`}>
                      {inv.status === "paid" ? (
                    ...
                    <span className="text-[14px] font-bold text-[#E8DFD0] tabular-nums shrink-0">
        ...
        <div className="space-y-4">
          {/* Payment Status */}
          <div className="bg-[#1F1B14] rounded-[6px] border border-[#312B20] shadow-[0_1px_3px_rgba(0,0,0,0.4)] p-5">
            <h3 className="text-[13px] font-bold text-[#E8DFD0] mb-4">
              Invoice Status
            </h3>
            ...
            {invoiceCount === 0 && (
              <p className="text-[12px] text-[#4E4235] text-center py-4">
                No invoices yet
              </p>
            )}
          </div>
          ...
          <div className="bg-[#1F1B14] rounded-[6px] border border-[#312B20] shadow-[0_1px_3px_rgba(0,0,0,0.4)] p-5">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-[13px] font-bold text-[#E8DFD0]">
                Open Estimates
              </h3>
              <Link
                href="/dashboard/estimates"
                className="text-[11px] font-semibold text-[#5A5040] hover:text-[#E8DFD0] transition-colors"
              >
                View All
              </Link>
            </div>
            <div className="flex items-baseline gap-2">
              <span className="text-[24px] font-normal text-[#E8DFD0] tabular-nums">
                {formatCurrency(totalEstimatesValue)}
              </span>
            </div>
            <p className="text-[12px] text-[#5A5040] mt-1">
          ...
          <div className="space-y-2">
            <Link
              href="/dashboard/invoices"
              className="flex items-center justify-between p-3.5 bg-[#1F1B14] rounded-[6px] border border-[#312B20] hover:border-[#312B20] hover:bg-[#262118] hover:shadow-[0_2px_6px_rgba(0,0,0,0.4)] transition-all group"
            >
              <div className="flex items-center gap-3">
                <div className="h-8 w-8 rounded-[6px] bg-white/[0.04] flex items-center justify-center">
                  <Receipt className="h-4 w-4 text-[#8C7D6A]" strokeWidth={1.8} />
                </div>
                <span className="text-[13px] font-semibold text-[#8C7D6A]">
                  Create Invoice
                </span>
              </div>
              <ArrowRight className="h-3.5 w-3.5 text-[#4E4235] group-hover:text-[#8C7D6A] transition-colors" strokeWidth={1.8} />
            </Link>

            <Link
              href="/dashboard/estimates"
              className="flex items-center justify-between p-3.5 bg-[#1F1B14] rounded-[6px] border border-[#312B20] hover:bg-[#262118] hover:shadow-[0_2px_6px_rgba(0,0,0,0.4)] transition-all group"
            >
              <div className="flex items-center gap-3">
                <div className="h-8 w-8 rounded-[6px] bg-white/[0.04] flex items-center justify-center">
                  <FileText className="h-4 w-4 text-[#8C7D6A]" strokeWidth={1.8} />
                </div>
                <span className="text-[13px] font-semibold text-[#8C7D6A]">
                  New Estimate
                </span>
...
function StatCard({
  ...
  return (
    <div className="bg-[#1F1B14] rounded-[6px] border border-[#312B20] shadow-[0_1px_3px_rgba(0,0,0,0.4)] p-4">
      <div className="flex items-center justify-between mb-3">
        <div className={`h-8 w-8 rounded-[6px] flex items-center justify-center ${
          alert ? "bg-red-500/100/10" : "bg-white/[0.04]"
        }`}>
          <Icon className={`h-4 w-4 ${alert ? "text-red-400" : "text-[#8C7D6A]"}`} strokeWidth={1.8} />
        </div>
        {alert && (
          <AlertCircle className="h-3.5 w-3.5 text-red-400" strokeWidth={1.8} />
        )}
      </div>
      <p className="text-[20px] font-normal text-[#E8DFD0] tabular-nums">
        {value}
      </p>
      <div className="flex items-center justify-between mt-1">
        <p className="text-[11px] text-[#5A5040] font-medium">
          {label}
        </p>
        {detail && (
          <p className="text-[10px] text-[#4E4235]">
            {detail}
          </p>
        )}
      </div>
...
function StatusRow({
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between">
        <span className="text-[12px] text-[#8C7D6A] font-medium">
          {label}
        </span>
        <span className="text-[12px] text-[#5A5040] tabular-nums">
          {count}
        </span>
      </div>
